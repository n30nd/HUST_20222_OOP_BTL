...
public class GUI7_3ToPDFConverter {
    public void convertToPDF(Component component1, Component component2, String filePath, String password) {
        try {
            // Tạo một BufferedImage cho từng thành phần giao diện
            BufferedImage image1 = new BufferedImage(component1.getWidth(), component1.getHeight(), BufferedImage.TYPE_INT_RGB);
            Graphics2D graphics1 = image1.createGraphics();
            ...
            // Cài đặt mật khẩu cho file PDF
            AccessPermission accessPermission = new AccessPermission();
            accessPermission.setCanPrint(true); // Cho phép in ấn
            accessPermission.setCanModify(false); // Không cho phép chỉnh sửa
            StandardProtectionPolicy protectionPolicy = new StandardProtectionPolicy(password, password, accessPermission);
            document.protect(protectionPolicy);

            // Thêm thông tin tài liệu (tùy chọn)
            PDDocumentInformation documentInformation = document.getDocumentInformation();
            documentInformation.setTitle("My PDF Document");
            documentInformation.setAuthor("Your Name");
            documentInformation.setSubject("PDF created with Apache PDFBox");
            documentInformation.setKeywords("PDF, Apache PDFBox");
            documentInformation.setCreator("Your Application");
